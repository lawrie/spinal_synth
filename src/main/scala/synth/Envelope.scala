...
class Envelope(accumulatorBits : Int = 26, sampleClkFreq: Int = 44100)  extends Component {
  ...
  val sampleArea = new ClockingArea(sampleDomain) {
    ...
    val accumulator = Reg(UInt(accumulatorBits + 1 bits))
    ...
    io.amplitude := amplitude addTag(crossClockDomain)

    val sustainVolume = io.s @@ U"0000"

    val sustainGap = 255 - sustainVolume

    val nextState = Reg(op)

    prevGate := io.gate addTag(crossClockDomain)
    ...
    when (overflow) {
      ...
      switch(state) {
        ...
        is(op.Decay) {
          ...
          decTmp := (((expOut * sustainGap) |>> 8) + sustainVolume).resized
